...
def process_segments_depth(depth_map, segment):
    try:
        ...
        segment_depths = depth_map[mask == 1]
        if segment_depths.size == 0:
            return None

        # 随机采样50个深度值
        if segment_depths.size >= 50:
            sampled_depths = np.random.choice(segment_depths, 50, replace=False)
        else:
            sampled_depths = segment_depths

        std_dev = np.std(sampled_depths)
        median_depth = np.median(sampled_depths)
        valid_depths = [depth for depth in sampled_depths if abs(depth - median_depth) <= 0.5 * std_dev]
...
class ObjectInfoPublisher(Node):
    def __init__(self):
        ...
        self.get_logger().info('ObjectInfoPublisher node has started.')

    def publish_message(self, msg):
        # 构造消息内容
        message_content = f"{msg}"
        msg = String()
        msg.data = message_content

        # 发布消息
        self.publisher_.publish(msg)
        self.get_logger().info(f'Published: {message_content}')
...
if __name__ == '__main__':
    ...
    start_to_convey = False
    
    rclpy.init(args=None)
    node = ObjectInfoPublisher()
    ...
    while cap.isOpened():
        ...
        if success:
            ...
            if rclpy.ok() and start_to_convey:
              # 存放每个框和深度信息的列表
                messages = []
                for i, (box, depth) in enumerate(zip(boxes, segment_depths)):
                    # 提取框的坐标和深度值
                    object_id = i + 1  # 假设每个物体的 ID 是递增的
                    cls_ = int(box[-1])
                    object_type = classes.get(cls_,"unknown")  # 示例物体类型
                    x1, y1, x2, y2 = int(box[0]), int(box[1]), int(box[2]), int(box[3])  # 将坐标转换为整数
                    depth /= 1000  # 转换为米
                    message = f"ID:{object_id}, Type:{object_type}, Coordinates:({x1},{y1}),({x2},{y2}), Depth:{depth:.2f} m "
